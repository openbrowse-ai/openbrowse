...
export function parseSource(input: string): ParsedSource {
  ...
  if (trimmed.startsWith("github:")) {
    ...
    if (parts.length < 2) {
      return {
        kind: "invalid",
        reason: "Invalid github: shorthand. Expected github:owner/repo",
      };
    }
    const owner = parts[0];
    const repo = parts[1];
  ...
  if (trimmed.startsWith("https://github.com/")) {
    try {
      ...
      if (parts.length < 2) {
        return {
          kind: "invalid",
          reason: "Invalid GitHub URL. Expected https://github.com/owner/repo",
        };
      }
  ...
  // 4. fallback: owner/repo shorthand without github: prefix
  ...
  return {
    kind: "invalid",
    reason: "Unrecognized source format. Use owner/repo or a GitHub URL.",
  };
}
