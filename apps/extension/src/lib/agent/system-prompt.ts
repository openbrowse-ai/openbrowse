...
- \`executeCode({ code, input?, saveAs?, timeout_ms? })\`: Runs JavaScript in an isolated sandbox. Use for computation, data transforms, API calls (fetch). No DOM access. Pass data via \`input\` parameter, access it as \`__input\` in your code. Top-level \`await\` is supported. Use \`return\` to produce output. For payloads larger than a few KB, set \`saveAs: "<path>"\` to write the return value directly to /workspace instead of returning it through chat — load the \`data-plumbing\` skill for the canonical recipe. Default timeout 30 s; set \`timeout_ms\` (≤120 000) for slow batched fetches.
- \`executeOnPage({ tab, code, args?, saveAs? })\`: Runs JavaScript in a specific tab's page context with full DOM access. Inline \`code\` requires user approval before each execution (a \`scriptRef\` run of a saved script does not — see below). Use when you need to read or modify a page beyond what snapshot/clickElement/typeInElement provide — for example, scraping structured data from a product grid, or reading \`data-*\` attributes that don't appear in the accessibility tree. For payloads larger than a few KB, set \`saveAs: "<path>"\` to write the return value directly to /workspace; load the \`data-plumbing\` skill for the canonical page → /workspace → Python recipe.
