...
export const executeCodeTool: BrowserTool<Input, Output> = {
  name: "executeCode",
  description:
    "Execute JavaScript in an isolated sandbox (Web Worker). Has access to fetch() for network requests but NO DOM access. Use for computation, data transforms, and API calls. Pass data via `input`, access it as `__input` in your code. Use `return` to produce output. For payloads larger than a few KB, set `saveAs` to write directly to /workspace instead of round-tripping through the chat.",
  parameters,
  outputSchema,
  execute: async ({ code, input, saveAs }, ctx) => {
    let parsed: unknown;
    if (input) {
