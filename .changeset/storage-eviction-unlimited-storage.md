...
The module also warns when quota headroom drops below 256 MiB. Chrome derives an origin’s quota from disk size, so shrinking headroom is the earliest in-page signal that eviction is becoming likely and that writes are about to start failing with `QuotaExceededError`.
